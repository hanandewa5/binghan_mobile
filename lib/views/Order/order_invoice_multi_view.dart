...
class OrderInvoiceMultiView extends StatefulWidget {
  const OrderInvoiceMultiView({Key key}) : super(key: key);

  @override
  _OrderInvoiceMultiViewState createState() => _OrderInvoiceMultiViewState();
}
...
class _OrderInvoiceMultiViewState extends State<OrderInvoiceMultiView> {
  ...
  Widget cardList(
      OrderViewModel model, int i, double _width, Color _colorAccent) {
    return InkWell(
      onTap: () {
                ...
                : model.listInvoiceMulti[i].status == "Pesanan Dikirim"
                    ? Column(
                        children: <Widget>[
                          ...
                          InkWell(
                            onTap: () {
                              model.setComplete(model.listInvoiceMulti[i].id);
                            },
                            child: Container(
                                width: double.infinity,
                                decoration: BoxDecoration(
                                    border: Border.all(
                                        width: 3, color: _colorAccent)),
                                padding: UIHelper.marginSymmetric(5, 8),
                                child: Paragraft(
                                  text: "Selesai",
                                  textStyle: textMedium,
